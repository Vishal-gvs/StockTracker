...
import { LogOut, Download, CheckCircle, PackagePlus, Edit } from 'lucide-react';
...
// Modal Component
const ItemModal = ({ onClose, onSuccess, initialData }: { 
    onClose: () => void, 
    onSuccess: () => void,
    initialData?: { _id: string; name: string; availableStock: number; costPerUnit: number }
}) => {
    ...
    const mutation = useMutation({
        mutationFn: async (data: { name: string; availableStock: number; costPerUnit: number }) => {
            if (initialData) {
                return api.put(`/items/${initialData._id}`, data);
            }
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center p-4">
            <div className="bg-white p-5 rounded-lg shadow-xl w-full max-w-lg">
                ...
                <form onSubmit={handleSubmit((data) => mutation.mutate({ ...data, availableStock: Number(data.availableStock), costPerUnit: Number(data.costPerUnit) }))}>
                    ...
                    <div className="mb-4">
                        <label className="block text-gray-700 text-sm font-bold mb-2">{initialData ? 'Available Stock (update if needed)' : 'Initial Stock'}</label>
                    ...
                    <div className="flex justify-end space-x-2">
                        ...
                        <button type="submit" className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">{initialData ? 'Update' : 'Save'}</button>
                    </div>
                </form>
            </div>
        </div>
...
const AdminExpenditure = () => {
  ...
  const [editingItem, setEditingItem] = useState<{ _id: string, name: string, availableStock: number, costPerUnit: number } | null>(null);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
          {activeTab === 'daily' ? (
              <>
                <div className="px-4 py-4 flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-4 sm:space-y-0">
                    ...
                    <div className="w-full sm:w-auto">
                        <button 
                            ...
                            className="flex items-center justify-center bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 w-full sm:w-auto"
                        >
              ...
              <>
                 <div className="px-4 py-4 flex justify-between items-center">
                    ...
                    >
                        <PackagePlus className="w-4 h-4 mr-2"/> <span className="hidden sm:inline">Add New Item</span><span className="sm:hidden">Add</span>
                    </button>
                 </div>
                 <div className="bg-white shadow overflow-hidden sm:rounded-lg overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
